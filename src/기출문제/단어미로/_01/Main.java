...
class UserSolution {
	...
	HashMap<Integer, ArrayList<String>> postMap2;
	HashMap<Integer, ArrayList<String>> postMap4;
	HashMap<Integer, ArrayList<String>> midMap;
	HashMap<Integer, ArrayList<String>> preMap2;
	HashMap<Integer, ArrayList<String>> preMap4;
	...
	void setMap(int seq, String word, HashMap<Integer, ArrayList<String>> map) {
		ArrayList<String> set = map.getOrDefault(seq, new ArrayList<String>());
		set.add(word);
		map.put(seq, set);
	}
	...
	int moveDir(int mDir) {
		...
		HashMap<Integer, ArrayList<String>> target = getTargetMap(mDir, findingWord.length());
		
		ArrayList<String> list = target.get(findingSeq);
		if(list.size() == 0) return 0;
		
		String foundWord = "zzzzzzzz";
		int foundId = 0;
		if(list.size() == 1) {
			foundWord = list.get(0);
			foundId = idByWord.get(foundWord);
			if(foundId == curWord.id) return 0;
			current = foundId;
			return current;
		}
		
		for(String word : list) {
			if(word.equals(curWord.word)) continue;
	...
	HashMap<Integer, ArrayList<String>> getTargetMap(int dir, int length) {
		switch (dir) {
			case 0:
