...
class UserSolution {	
	...
	public int conflict(char[] mCompany1, char[] mCompany2) {
		...
		int pSeq1 = find(seq1);	
		int pSeq2 = find(seq2);	
		HashSet<Integer> relatedCompany1 = new HashSet<>();
		HashSet<Integer> relatedCompany2 = new HashSet<>();
		
		int[] used = new int[companyCount + 1];
		HashSet<Integer> companySet1 = companyInfo[pSeq1].companies;
		for(int seq : companySet1) {			
			for(int related : graph[seq]) {
				if(used[related] == 1) continue;			
