...
public class Main {
	...
	static int bfs() {
		...
		q.add(new Node(0,0,false, fVisited , "(0,0)(0)"));
		fVisited[0][0] = 0;
		...
		while(!q.isEmpty()) {
			...
			for(int i=0; i<4; i++) {
				...
				if(map[nextRow][nextCol] == 4) {
					...
					int sCount = 1;
					while(continualble) {						
						newRow += directRow[i] * 2;
						newCol += directCol[i] * 2;
						sCount += 2;
						if(impossibleDir(newRow, newCol)) {
							...
							for(int k=2; k> 0; k--) {
								...
								newCol -= directCol[i];
								sCount -= 1;	
								if(!impossibleDir(newRow, newCol)) break;
							}
						} else {
							if(map[newRow][newCol] != 4) {
								continualble = false;
								int prevRow = newRow - directRow[i];
								int prevCol = newCol - directCol[i];
								if(map[prevRow][prevCol] == 4) {
								...
								if(map[newRow][newCol] == 1 || map[newRow][newCol] == 2) {
									...
									} else if(map[prevRow][prevCol] == 3 || map[prevRow][prevCol] == 0) {
										newRow = prevRow - directRow[i];
										newCol = prevCol - directCol[i];
										sCount -= 2;
									} else if(map[prevRow][prevCol] == 1 || map[prevRow][prevCol] == 2) {
										newRow = prevRow;
										newCol = prevCol;
										sCount -= 1;
									}
									slided[prevRow][prevCol][dir] = 1;
								}
								if(map[newRow][newCol] == 3 || map[newRow][newCol] == 0) {
									newRow -= directRow[i];
									newCol -= directCol[i];
