...
class UserSolution {
	...
	final int GATE_CONSTANT = 1000;
	int N;
	int maxStemina;
	int[][] map;
	int[] removed;
	
	ArrayList<Gate>[] graph;
			...
			for(int i=0; i<4; i++) {
				...
				if(foundGate != 0) {					
					if(removed[foundGate] != 1) {
						graph[foundGate].add(new Gate(id, cur.count + 1));
						graph[id].add(new Gate(foundGate, cur.count + 1));
					}				
				}
	...
	public int getMinTime(int mStartGateID, int mEndGateID) {
		...
		PriorityQueue<Gate> q = new PriorityQueue<>((o1, o2) -> Integer.compare(o1.cost, o2.cost));
