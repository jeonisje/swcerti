...
class UserSolution {	
	...
	HashMap<String, Integer> scoreByStr;
	
	ArrayList<Integer>[] graph;	
	ArrayList<String>[] restauarantByTown;
	...
	public void init(int N, int M, int mRoads[][]) {			
		...
		scoreByStr = new HashMap<>();
		graph = new ArrayList[N+1];
		visited = new int[N+1];
		restauarantByTown = new ArrayList[N+1];
		for(int i=0; i<N+1; i++) {
			graph[i] = new ArrayList<>();
			restauarantByTown[i] = new ArrayList<>();
		}				
	...
	public void addRestaurant(int mCityID, char mName[]) {
		...
		restauarantByTown[mCityID].add(s);
      	return; 
	}	
	...
	public void addValue(char mName[], int mScore) {
		...
		int curScore = scoreByStr.get(s) + mScore;
		scoreByStr.put(s, curScore);	
		int score = 0;
		for(int k=0; k < mName.length; k++ ) {
			for(int i=0; i < mName.length-k; i++) {
				String s1 = s.substring(i, i+1+k);	
				score = bestByStr.get(s1);
				bestByStr.put(s1, Math.max(curScore, score));
			}
		}		
      	return; 
	}
	...
	public int regionalValue(int mCityID, int mDist) {
		...
		while(!q.isEmpty()) {
			...
			for(String s : restauarantByTown[cur.id]) {
				pq.add(scoreByStr.get(s));
			}
			
			if(cur.dist == mDist) continue;
			
			for(int next : graph[cur.id]) {
				if(visited[next] == cy) continue;
