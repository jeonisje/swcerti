package 기출문제.화물운송.parameticSearch;
...
class UserSolution {
	...
	boolean bfs(int limit, int start, int end) {
		...
		while(!q.isEmpty()) {
			...
			for(Node next : graph[cur.id]) {				
				if(visited[next.id] == count) continue;
				if(next.limit < limit) continue;
				
				q.add(next);
				visited[next.id] = count;
			}
		}
