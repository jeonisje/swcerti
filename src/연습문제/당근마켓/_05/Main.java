...
class UserSolution {
	...
	public int sellCarrot(String tag1, String tag2, String tag3) {
		...
		for(Map.Entry<Carrot, Integer> entry : searchMap.entrySet()) {
			int id = entry.getKey().id;
			if(selled[id] == 1) continue;
			int matched = 0;
			int count = 0;
			if(!tagsById[id].contains(findTags[0])) continue;
				for(String tag : tagsById[id]) {
					if(tag.equals(findTags[matched])) {
						matched++;
					} else if (tagsById[id].size() == 3 && count == 2) {
						 break;
					} else if (tagsById[id].size() == 4 && count == 3 && matched == 1) {
						 break;
					} else if (tagsById[id].size() == 5 && count == 4 && matched == 1) {
						 break;
					}
					if(matched == 3) {
						selled[id] = 1;
	...
	public void updatePrice(String tag1, int addPrice) {
		...
		for(int id: list) {
			...
			Carrot carrot = objectMappping[id];
			searchMap.remove(carrot);
			searchMap.put(carrot, id);
