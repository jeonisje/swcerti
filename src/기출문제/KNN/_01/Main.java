...
class UserSolution {
	...
	public void init(int K, int L) {
	    ...
	    countByCategory = new int[MAX_CATEGORY];
	    sequence = 0;
	}
	...
	public int predict(int mX, int mY) {
		...
		for(int i=0; i<K; i++) {
			KNN k = q.poll();
			countByCategory[k.category]++;
			
			if(countByCategory[k.category] < maxCategory) {
				maxCategory = countByCategory[k.category];
				ret = k.category;
			} 
